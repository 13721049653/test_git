...
public class MiddleInfoServiceImpl  implements MiddleInfoService{
    ...
    public void syncSfz_sjh(){
        ...
        for(UserInfo userInfo:userBaseUpdateList) {
            ...
            String sqlUpdateUser =  "update sys_user set telephone = '"+telephone+"' , card_id = '"+cardId+"' where login_name = '"+login_name+"'  ";
            jdbcTemplate.execute(sqlUpdateUser);
        }

        String sqlUpdateSpace = "";
        String [] relaParams = {"telephone","card_id","phone"};
        for(String param : relaParams){
            sqlUpdateSpace = " update sys_user set "+param+"=null where "+param+"=' ' ";
            jdbcTemplate.execute(sqlUpdateSpace);
        }
    ...
    public void deleteOrgUser(){
        String sqlOrg = "update sys_org set is_delete=1 where   org_code !='defaultCorp'  and (third_uid is not null or org_code in (select org_code from sys_org where org_code not in (select fnumber from XFSMZJK.T_KDXF_DEPARTMENT@toerp)))";
        //组织用户删除恢复
        String sqlOrgHF=" update sys_org  a  set is_delete =0 where is_delete=1 and  exists (select 1 from XFSMZJK.T_KDXF_DEPARTMENT@toerp b where b.fnumber = a.org_code)";
        jdbcTemplate.execute(sqlOrg);
        jdbcTemplate.execute(sqlOrgHF);
        ...
        String sqlUser = "update sys_user set is_delete =1 where (login_name not like '%whir%' and login_name not like '%admin%') and id in (select a.id from sys_user a inner join sys_org_user b on a.id=b.user_id inner join sys_org c on b.org_id = c.id where (a.login_name not in (select FUSERNAME from XFSMZJK.T_KDXF_STAFF@toerp where F_KDXF_JYZT = 'A' )  or a.card_id is null))";
        String sqlUserHF = "update sys_user set is_delete =0 where id in (select a.id from sys_user a inner join sys_org_user b on a.id=b.user_id inner join sys_org c on b.org_id = c.id where (a.login_name in (select FUSERNAME from XFSMZJK.T_KDXF_STAFF@toerp  where F_KDXF_JYZT = 'A') ))";
